...
int node_fit(const Node *x, const char *pattern) {
	return (!strncmp(x->his.val, pattern, x->his.size - 1));
}
...
void node_attach(Node *parent, Node *child) {
	...
	child->depth = parent->depth + parent->his.size - 1;
...
void node_split(Node *x, size_t prefix_size) {
	...
	added_child->his = hist_suffix(&old, old.size - prefix_size + 1);
	hist_destroy(&old);
...
Node * find_parent(Node *tree, const char *str) {
	...
	for (; next; next = *get_child(tree, str)) {
		if (next->his.val[0] != str[0])
			return previous;
		if (!node_fit(next, str))
			return tree;
		str += next->his.size - 1;
		if (str[0] == '\0')
			return tree;
...
//Node *new_find_parent(Node *tree, const char *str) {
//	Node *next = tree, *prev = tree;
//	for (;;) {
//		if (node_fit(next, str)) {
//			str += next->his.size - 1;
//			prev = tree;
//			tree = next;
//			next = *get_child(next, str);
//			continue;
//		}
//		if (*get_child(tree, str) == NULL)
//			return prev;
//
//		break;
//	}
...
Node * tree_find_exact(Node *tree, const char *str) {
	...
	str += parent->depth + parent->his.size - 1;
...
Node * tree_find_split(Node *tree, const char *str) {
	Node *child, *parent = find_parent(tree, str);
	str += parent->depth + parent->his.size - 1;
	child = *get_child(parent, str);
	if (child != NULL) {
		size_t len = strlen(str);
		if (strncmp(child->his.val, str, len - 1) != 0)
			return NULL;
		if (child->his.size == len)
			return child;
		node_split(child, len);
	}
	return child;
}
...
void tree_insert(Node *tree, const char *str) {
	Node *old_child;
	size_t prefix_size;
	tree = find_parent(tree, str);
	str += tree->depth + tree->his.size - 1;
	old_child = *get_child(tree, str);
	if (old_child == NULL) {
		node_attach(tree, node_init(str));
		return;
	}
	prefix_size = 1 + first_difference(&old_child->his, str);
	if (prefix_size >= 1 + strlen(str))
		return;
	str += prefix_size - 1;
	if (prefix_size >= old_child->his.size) {
		node_attach(old_child, node_init(str));
		return;
	}
	node_split(old_child, prefix_size);
	node_attach(old_child, node_init(str));
}

void tree_remove(Node *tree, const char *str) {
	tree_insert(tree, str);
	tree_find_split(tree, str);
	Node **child, *parent = find_parent(tree, str);
	str += parent->depth + parent->his.size - 1; //todo
	child = get_child(parent, str);
	if (!strncmp((*child)->his.val, str, strlen(str)))
